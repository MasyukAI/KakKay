...
describe('CartConditionCollection Coverage Tests', function () {
    ...
    it('can be merged with another collection gracefully', function () {
        $collection1 = new CartConditionCollection();
        $collection2 = new CartConditionCollection();
        
        $condition1 = new CartCondition('First', 'discount', 'subtotal', '-5');
        $condition2 = new CartCondition('Second', 'charge', 'subtotal', '10');
        
        $collection1->put('first', $condition1);
        $collection2->put('second', $condition2);
        
        $merged = $collection1->merge($collection2);
        
        expect($merged)->toHaveCount(2);
        
        // The merge preserves existing keys from the first collection
        // and uses condition names for new conditions
        expect($merged->has('first'))->toBeTrue(); // from collection1
        expect($merged->has('Second'))->toBeTrue(); // condition name from collection2
        
        expect($merged->first())->toBeInstanceOf(\MasyukAI\Cart\Conditions\CartCondition::class);
    });
...
it('can merge collections properly with condition instances', function () {
    $collection1 = new CartConditionCollection();
    $collection1->addCondition(new CartCondition('discount1', 'discount', 'subtotal', '-10'));
    
    $collection2 = new CartConditionCollection();
    $collection2->addCondition(new CartCondition('tax1', 'charge', 'subtotal', '5'));
    
    $merged = $collection1->merge($collection2);
    
    expect($merged->count())->toBe(2);
    expect($merged->get('discount1'))->toBeInstanceOf(CartCondition::class);
    expect($merged->get('tax1'))->toBeInstanceOf(CartCondition::class);
});

it('can merge with array of condition instances', function () {
    $collection = new CartConditionCollection();
    $collection->addCondition(new CartCondition('discount1', 'discount', 'subtotal', '-10'));
    
    $conditionsArray = [
        new CartCondition('tax1', 'charge', 'subtotal', '5')
    ];
    
    $merged = $collection->merge($conditionsArray);
