...
it('can be merged with another collection', function (): void {
    $otherCollection = new CartConditionCollection();
    
    $this->collection->put('discount-10', $this->condition1);
    $otherCollection->put('shipping', $this->condition2);
    $otherCollection->put('tax', $this->condition3);
    
    $merged = $this->collection->merge($otherCollection);
    
    expect($merged->count())->toBe(3)
        ->and($merged->has('discount-10'))->toBeTrue()
        ->and($merged->has('shipping'))->toBeTrue()
        ->and($merged->has('tax'))->toBeTrue();
});
