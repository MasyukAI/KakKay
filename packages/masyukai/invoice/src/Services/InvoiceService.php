...
namespace MasyukAI\Invoice\Services;
...
class InvoiceService
{
    ...
    {
        $config = config('invoice.number_format');
    ...
    public function createInvoice(InvoiceData $data): Invoice
    {
        ...
        $invoice = Invoice::create([
            ...
            'due_date' => $data->dueDate ?? now()->addDays(config('invoice.defaults.due_days', 30)),
            'subtotal' => $subtotal,
            'tax_amount' => $taxAmount,
            'discount_amount' => $discountAmount,
            'total' => $total,
            'currency' => $data->currency ?? config('invoice.defaults.currency', 'MYR'),
            'notes' => $data->notes,
            'terms' => $data->terms,
            'customer_data' => $data->customerData,
            'company_data' => $data->companyData ?? config('invoice.company'),
            'items' => $data->items,
    ...
    public function generatePdf(Invoice $invoice, bool $save = true): string
    {
        ...
        $viewName = $template?->view_name ?? config('invoice.default_template', 'default');

        $pdf = Pdf::view("invoice::templates.{$viewName}", [
            'invoice' => $invoice,
        ])
            ->format(config('invoice.pdf.format', 'a4'))
            ->orientation(config('invoice.pdf.orientation', 'portrait'))
            ->margins(
                config('invoice.pdf.margin.top', 10),
                config('invoice.pdf.margin.right', 10),
                config('invoice.pdf.margin.bottom', 10),
                config('invoice.pdf.margin.left', 10)
            );

        if ($save) {
            $path = $this->generatePdfPath($invoice);
            $disk = config('invoice.storage.disk', 'local');
    ...
    public function downloadPdf(Invoice $invoice): string
    {
        if ($invoice->pdf_path && Storage::disk(config('invoice.storage.disk', 'local'))->exists($invoice->pdf_path)) {
            return Storage::disk(config('invoice.storage.disk', 'local'))->url($invoice->pdf_path);
        }
    ...
    {
        $basePath = config('invoice.storage.path', 'invoices');
